        ...
        if (ctp.playerData.containsKey(event.getPlayer().getName())) {
            ...
            if (event.hasBlock() && event.getClickedBlock().getState() instanceof Sign) {
                ...
                if (role.equalsIgnoreCase("[CTP]")) {
            		if (event.getPlayer().getItemInHand().getType() != Material.AIR && event.getPlayer().getItemInHand().getType() != Material.WOOL){
            			ctp.sendMessage(event.getPlayer(), ChatColor.RED + "You must use an empty hand or wool!");
            			return;
            		}else{
            			shop(p, sign);
            		}
            		...
            		if (event.getPlayer().getItemInHand().getType() != Material.AIR){
            			ctp.sendMessage(event.getPlayer(), ChatColor.RED + "You must use an empty hand!");
            			return;
            		}
